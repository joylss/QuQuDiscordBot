...
public class CommandHandler
{
    ...
    public async Task HandleCommand(SocketMessage s)
    {
        ...
        string prefix = "ququ ";
        string prefix2 = "QuQu ";
        if (msg.HasStringPrefix(prefix, ref argPos) || msg.HasStringPrefix(prefix2, ref argPos) || msg.HasMentionPrefix(_client.CurrentUser, ref argPos))
        {
            ...
            if (!result.IsSuccess)
            {
                switch (result.ToString())
                {
                    ...
                    case "UnknownCommand: Unknown command.":
                        ...
                        await s.Channel.SendMessageAsync($"Command not found! Use the command qqhelp for a list of commands.");
                        break;
